...
def process_videos(json_path, output_dir):
    ...
    check_required_tools(REQUIRED_TOOLS["videos"])

    with open(json_path, "r") as f:
        checksum_map = json.load(f)

    os.makedirs(output_dir, exist_ok=True)
    log_path = create_timestamped_log_path(output_dir, "log_videos")
    init_logging(log_path)

    print(f"🎞️ Starting conversion of {len(checksum_map)} unique video files...")
